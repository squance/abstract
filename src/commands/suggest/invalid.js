...
module.exports = class Invalid extends Command {
    constructor(client) {
        super(client, {
            name: "invalid",
            description: "Rend une suggestion invalide.",
            args: true,
    ...
    async run(message, args, data) {
        ...
        const suggestioEmbed = suggestion.embeds[0];
        const newEmbed = {
            color: message.client.colors.noir,
            author: {
                name: suggestioEmbed.author.name,
                iconURL: suggestioEmbed.author.iconURL
            },
            description: suggestioEmbed.description ? suggestioEmbed.description : "",
            footer: {
                text: suggestioEmbed.footer.text
            },
            image: {
                url: suggestioEmbed.image ? suggestioEmbed.image.url : ""
            },
            fields: [
                {
                    name: "Suggestion invalide",
                    value: suggestioEmbed.fields[0].value
                },
            ]
        }
        if (reason.length) {
        ...
        await find.delete();
        message.channel.send(message.client.emotes.yup + " La suggestion a bien été considérée comme invalide.").then(msg => msg.delete({timeout: 5*1000}).catch(() => {}));
        message.delete(
            {
                timeout: 5*1000
            }

            ...
            member.send(
                {
                    embed: {
                        color: message.client.colors.vert,
                        description: `Votre [suggestion](${suggestion.url}) a été approuvée !`
                    }
                }
            ).catch(() => {});
        };
        await find.delete();
        message.channel.send(message.client.emotes.yup + " La suggestion a bien été acceptée.").then(msg => msg.delete({timeout: 5*1000}).catch(() => {}));
        message.delete(
            {
                timeout: 5*1000
            }
